...
uint256 hashGenesisBlock(" 0x81c6daf40bc8dcea9e99d6bf79d47b970ad5a6d0c658ce880a5b9095f37fc783");//
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Jtccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jtccoin Signed Message:\n";
...
{
    // Jtccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight==1)
        nSubsidy=88000000*COIN;
    else
        nSubsidy=0*COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30*60; // Jtccoin: 30min
static const int64 nTargetSpacing = 1 * 60; // Jtccoin:30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jtccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jtccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Jtccoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
      const char* pszTimestamp = "Created By zaobi.org .QQ:260682605 ,欢迎咨询!2016-8-15 8:00:00";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256(" 0x354d7478b014f4499600e7c612bb033bec8778cbc7381376e85a487c597ffd1e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Jtccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("jtccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("jtccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static jtccoinMiner(CWallet *pwallet)
{
    printf("jtccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jtccoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running jtccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("jtccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&jtccoinMiner, pwallet));
}
